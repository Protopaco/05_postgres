...
module.exports = class Teacher {
    ...
    static async insert({ host_id, user_name, email, pic_url, access_token, account_id, last_update }) {
        const { rows } = await pool.query(
            'INSERT INTO teachers ( host_id, user_name, email, pic_url, access_token, account_id, last_update)
    ...
    const async update(id, { host_id, user_name, email, pic_url, access_token, account_id, last_update }) {
        const { rows } = await pool.query(
            'UPDATE teachers
            SET 
            host_id = $1,
